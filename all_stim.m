...
stimtime = 1.5 ; % stimulus time in seconds
resttime = 0.75 ; % post stimulus time
...
trigs = [21:22,1:20,41:56] ; %21:36,
prots = [90,180] ; % arots(end) = [] ; 
randparams = repmat(trigs,[1,30]) ; 
...
% make the plaids: 
[xg,yg] = meshgrid(-350:.125:350,-50:.125:50) ; 
sinxg = sin(xg) ; 
binsin = sinxg <0 ; 
clear tmat ; 
tmat(:,:,2) = uint8(zeros(size(xg))+255) ; 
...
clear tmat ; 
tmat(:,:,2) = uint8(zeros(size(xg))+255) ; 
tmat(:,:,1) = uint8(mat2gray(sinxg)*255) ; 
...
for presentation = 1:length(randparams)
    ...
    stimcount = 1 ;
    poststimcount = 1 ;
    jitter = (resttime*frameRate) + (rand-.5)*frameRate ; 
    DONE = false ; 
    trigsent = false ; 
    stimarr = stimarrs(1,:) ; 
    currentround = 0 ; 
    while ~DONE             
        ...
        if currentstate == 2 % present the stimulus               
            ...
            srcRect=[0+xoffset/2 0 screenRect(3)+xoffset/2 screenRect(4)];   
            ...
            elseif randparams(presentation) < 40
                ...
                Screen('DrawTexture',w,sintex,srcRect,sqrRect,rots(randparams(presentation)-40)) ;                
            end
