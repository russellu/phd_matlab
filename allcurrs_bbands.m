...
for ccy=14%:length(currs)
    ...
    d5 = dkdata{5} ;
    mvgperiods = [15:5:90]; 
    for mvgperiod=1:length(mvgperiods)

        mvg = getmvg(d5,mvgperiods(mvgperiod)); 
        bbands = zeros(2,length(d5)); 
        for i=mvgperiods(mvgperiod)+1:length(d5)
            stdi = std(d5(i-mvgperiods(mvgperiod):i)); 
            bbands(1,i) = mvg(i)+stdi*2;
            bbands(2,i) = mvg(i)-stdi*2; 
        end
        ...
        for tpcount=1:length(tps) ; disp(['tpcount = ',num2str(tpcount), ' currency = ', currs{ccy}]); 
            for stopcount=1:length(stops)
                ...
                for i=2000:length(d5)
                    ...
                    if d5(i-1)>bbands(2,i) && d5(i)< bbands(2,i)  
                        sell = true; entry = d5(i); ntrades = ntrades + 1; 
                    elseif d5(i-1)<bbands(1,i) && d5(i) > bbands(1,i) 
        ...
        allcorrs(ccy,mvgperiod,:,:) = corrs; allmeanprofits(ccy,mvgperiod,:,:) = meanprofits; 
    end
end
