...
elayers = load_untouch_nii('native_layers.nii.gz') ;  eimg = elayers.img ;   
...
expo = 1 ; 
rgbs(:,:,3) = uint8(mat2gray(squeeze(mean(dqs(5,:,:),1)).^expo)*255) ;  
rgbs(:,:,2) = uint8(mat2gray(squeeze(mean(dqs(3:4,:,:),1)).^expo)*255) ;  
rgbs(:,:,1) = uint8(mat2gray(squeeze(mean(dqs(2,:,:),1)).^expo)*255) ;  
figure,imagesc(rgbs)
allrgbs(el,:,:,:) = rgbs ; 
allvqs(el,:,:,:) = vqs ; 
imwrite(rgbs,['c:/shared/UTE_pngs/',allelecs(el).name,'.png']) ;
...
skinter = skinlines.*skin ; cskinter = imdilate(cskinlines.*skin,strel(ones(7,7,7))) ; 
