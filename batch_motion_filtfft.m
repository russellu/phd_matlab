...
for j=1:length(freqs)
    filts(:,j,:) = eegfiltfft(ica.icaact,250,freqs(j)-3,freqs(j)+3) ; disp(j) ; 
end

mfilts = squeeze(mean(mean(abs(filts(comps{sub},:,10000:end-10000)),1),3)) ; 
allmfilts(sub,:) = mfilts ; 
...
allots = moviemix(:,occ{subns(sub)}) ;
[sb,fb] = spectopo(allots',0,1/0.693,'plot','off') ;
allsb(sub,:) = mean(sb,1) ; 
...
for i=1:size(allsubcorrs,1)
    meancorrs(i,:,:) = squeeze(mean(allsubcorrs(i,comps{subns(i)}(:),:,:),2)) ; 
end
figure,for i=1:8 ; subplot(3,3,i) ;imagesc(squeeze(meancorrs(i,:,:))) ; axis xy ; end
figure,imagesc(squeeze(mean(meancorrs,1)),[-.25,.25]) ; axis xy ;
...
save('mean_moviecorrs','meancorrs') ; 
save('movie_allmfilts','allmfilts') ; 
save('movie_sb','allsb') ; 
save('movie_specs','meanspecs') ; 
%}