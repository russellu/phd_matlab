...
for ccy=1:length(currs)
   % if(ccy>19)
        %cd(['e:\dukas\past3_forex\',timeframes{1}]) 
   ...
   % cci = getcci(lows,highs,closes,period);
   ...
   % for i=2:length(cci) ; if cci(i) > 150 && cci(i-1)<150 ; indi(i) = -1 ; elseif cci(i) < -150 && cci(i-1) > -150 ; indi(i) = 1; end ; end
    ...
    tps = [50,100,150]; 
    periods = [40,50,60,80,110,140,200,250,350,500,800]; 
    
    clear allprofit allntrades alltradetimes; 
    for t1=1:length(periods)
        for t2=1:length(tps)
       
    %bbands = getbbands(opens,periods(t1),2); 
    ...
    for i=2:length(aroon) ; if aroon(i) > 70  ; indi(i) = -1 ; elseif aroon(i) < -70 ; indi(i) = 1; end ; end %&& aroon(i-1) < 98 && aroon(i-1) > -98 
    
    close_ask = dkask{2}; 
    close_bid = dkbid{2}; 
    
    in_buy = false; in_sell = false; entry_price = 0; profit = 0; ntrades = 0; saved_trades = []; entry_index = 0; tradetime = 0; nightprofit = 0; 
    for i=800:length(opens)        
        if ~in_buy && ~in_sell 
            if indi(i) == 1 
...
for i=1:size(currprofits,2)
    for j=1:size(currprofits,3)
        ...
        subplottight(size(currprofits,2),size(currprofits,3),count); plot(squeeze(mean(currcumsums(si(1:20),i,j,:),1))); set(gca,'XTickLabel',[],'YTickLabel',[]); 
        count = count + 1 ; 
        mcorrs(i,j) = corr2(1:500,squeeze(mean(currcumsums(si(1:20),i,j,:),1))'); 
    end
end
