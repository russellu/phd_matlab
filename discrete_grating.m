...
circmask = double(sqrt(xg.^2 + yg.^2) < 150);
...
for presentation = 1:length(randparams)
    ...
    while ~DONE             
        ...
        if currentstate == 2 % present the stimulus               
          ...
          %      Screen('DrawTexture',w,checktex1,[],sqrRect) ;                
          %  else
          %      Screen('DrawTexture',w,checktex2,[],sqrRect) ;
          %  end
            Screen('DrawTexture',w,gratetex(mod(stimcount,51)+1),[],sqrRect) ;
            Screen('DrawTexture',w,wedgetex,[],sqrRect,rots(presentation)) ; 
