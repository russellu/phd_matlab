...
rute = load_untouch_nii('gs_20_1.nii.gz') ; ruteorig = double(rute.img) ; %ruteorig = ruteorig - imfilter(ruteorig,fspecial('gaussian',80,40)); 
z2 = load_untouch_nii('ute_mask.nii.gz') ;  z2.img = medfilt3(z2.img) ; %imdilate(medfilt3(imerode(double(z2.img),strel(ones(5,5,5)))),strel(ones(5,5,5))) ; 
...
for i=1:size(vqs,1) ; dqs(i,:,:) = mat2gray((squeeze(vqs(i,:,:))) - imfilter((squeeze(vqs(i,:,:))),fspecial('gaussian',35,20))) ; end
%dqs = vqs ; 
th = 5; 
for i=1:6
   dqi = squeeze(dqs(i,:,:)) ;  
...
expon = 1.5 ; 
clear rgbs imrotate
rgbs(:,:,3) = imrotate((uint8(mat2gray(im1).^expon*255)),0) ; rgbs(:,:,2) = imrotate((uint8(mat2gray(im2).^expon*255)),0) ; rgbs(:,:,1) = imrotate((uint8(mat2gray(im3).^expon*255)),0) ; 
figure,
