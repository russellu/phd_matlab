...
function [P,R,mbase,timesout,freqs,Pboot,Rboot,alltfX,PA] = nonlogtimef( data, frames, tlimits, Fs, varwin, varargin);
...
% ---------------------------
if strcmpi(g.scale, 'log')
    if ~isnan( g.baseline(1) ) && ~isnan( mbase(1) ) && strcmpi(g.trialbase, 'off'), mbase = (mbase); end;
    %%%% THIS PARTY
    P = (P);
    if ~isempty(Pboot)
        Pboot = (Pboot);
