...
for ccy=1:length(currs)
    ...
    threshs = [1,1.5,2,2.5,3]; 
    periods = [150:100:700]; 
    aroon = getaroon(opens,350); 
    indi = zeros(1,length(aroon)); 
    for i=2:length(aroon) ; if aroon(i) > 95  ; indi(i) = -1 ; elseif aroon(i) < -95 ; indi(i) = 1; end ; end %&& aroon(i-1) < 98 && aroon(i-1) > -98 
    ...
    for t1=1:length(periods)
      ...
      bbands = getbbands(opens,periods(t1),threshs(t2)); 
      filt_indi = zeros(1,length(bbands)); 
      for i=2:length(bbands) ; if opens(i) > bbands(1,i) ; filt_indi(i) = -1; elseif opens(i) < bbands(2,i) ; filt_indi(i) = 1 ; end ; end 
    ...
    for i=1200:length(opens)        
        if ~in_buy && ~in_sell 
            if indi(i) == 1 && filt_indi(i)==1
                in_buy = true; entry_price = ask_price(i); ntrades = ntrades + 1; entry_index = i; 
            elseif indi(i) == -1 && filt_indi(i)==-1
                in_sell = true; entry_price = bid_price(i); ntrades = ntrades + 1; entry_index = i; 
            end           
        elseif in_buy
            upl = (bid_price(i) - entry_price)/scales(ccy) ;
            if (indi(i)==-1 && filt_indi(i)==-1) || upl > 250
                profit = profit + bid_price(i) - entry_price ; in_buy = false; tradetime = tradetime + i-entry_index; 
                saved_trades(length(saved_trades)+1) = (bid_price(i) - entry_price) ./ scales(ccy) ; 
            end
        elseif in_sell
            upl = (entry_price - ask_price(i))/scales(ccy);
            if (indi(i)==1 && filt_indi(i)==1) || upl > 250
                profit = profit + entry_price - ask_price(i); in_sell = false; tradetime = tradetime + i-entry_index;
                saved_trades(length(saved_trades)+1) = (entry_price - ask_price(i)) ./ scales(ccy) ; 
            end
        end    
...
for i=1:size(currprofits,2)
    for j=1:size(currprofits,3)
        ...
        subplottight(size(currprofits,2),size(currprofits,3),count); plot(squeeze(mean(currcumsums(si(1:15),i,j,:),1))); set(gca,'XTickLabel',[],'YTickLabel',[]); 
        count = count + 1 ; 
        mcorrs(i,j) = corr2(1:500,squeeze(mean(currcumsums(si(1:15),i,j,:),1))'); 
    end
end
