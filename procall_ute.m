...
cd(['c:/shared/ute/',mongs(m).name]) ; ls 
disp('loading raw data...') ; 
rute = load_untouch_nii('res_ute.nii.gz') ; ruteorig = rute.img ; 
rutime = rute.img ; 
...
strip = zeros(size(allrgbs,1),size(allrgbs,2)*8,3) ; clear strip ; 
for i=1:8 ; strip(:,(i-1)*size(allrgbs,2)+1:(i-1)*size(allrgbs,2)+size(allrgbs,2),:) = squeeze(allrgbs(:,:,:,i)) ; end
imagesc(strip) ; set(gca,'XTick',[],'YTick',[]) ; 
