...
epoch1 = load_untouch_nii('epoch1.nii') ; 
epoch2 = load_untouch_nii('epoch2.nii') ; 
epoch3 = load_untouch_nii('epoch3.nii') ; 
epoch4 = load_untouch_nii('epoch4.nii') ; 
epoch5 = load_untouch_nii('epoch5.nii') ; 

stimtime = 20 ; % stimulus time in seconds 
resttime = 0 ; % post stimulus time 
focustime = 0 ; % minimum focus time (+ 1-2s jitter)
nparams = 5 ; % 6 orientations for the starting angle, and the 2 annulus types
ntrials = 4 ;
...
for i=1:size(epoch1.img,3)
    etex(:,:,1) = squeeze(epoch1.img(:,:,i)) ; 
    etex(:,:,2) = white ; 
    movtex(1,i) = Screen('MakeTexture',w,etex) ; 
end
for i=1:size(epoch2.img,3)
    etex(:,:,1) = squeeze(epoch2.img(:,:,i)) ; 
    etex(:,:,2) = white ; 
    movtex(2,i) = Screen('MakeTexture',w,etex) ; 
end
for i=1:size(epoch3.img,3)
    etex(:,:,1) = squeeze(epoch3.img(:,:,i)) ; 
    etex(:,:,2) = white ; 
    movtex(3,i) = Screen('MakeTexture',w,etex) ; 
end
for i=1:size(epoch4.img,3)
    etex(:,:,1) = squeeze(epoch4.img(:,:,i)) ; 
    etex(:,:,2) = white ; 
    movtex(4,i) = Screen('MakeTexture',w,etex) ; 
end
for i=1:size(epoch5.img,3)
    etex(:,:,1) = squeeze(epoch5.img(:,:,i)) ; 
    etex(:,:,2) = white ; 
    movtex(5,i) = Screen('MakeTexture',w,etex) ; 
end
...
for presentation = 1:length(stimtrigs) ; 
    ...
    while ~DONE             
        ...
        if currentstate == 2 % present the stimulus  
            ...
            Screen('DrawTexture',w,movtex(stimtrigs(presentation),ceil(stimcount/2)),[],[0,0,screenWidth,screenHeight]) ; 
            Screen('DrawTexture',w,xhairtex) ; 
            Screen('DrawTexture',w,xhairsmalltex) ;             
            Screen('DrawTexture',w,xhairsmallesttex) ;  
