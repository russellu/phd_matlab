...
for st=1:2 
    ...
    if st==2 ; box = fliplr(box) ; end
    inds = 1:numel(box) ; 
    indvals = box(inds) ; 
    [~,sortinds] = sort(indvals,'ascend') ; 
    [wx,wy] = ind2sub(size(box),sortinds) ;
    wx = wx*(screenRect(3)./max(wx)) ; wy = wy*(screenRect(4)./max(wy)) ; 
    dists = sqrt((wx-screenRect(3)./2).^2 + (wy - screenRect(4)./2).^2) ;
    dists = mat2gray(dists)*300 ; 
    
    % increase the sampling density in the fovea: 
    reswx = imresize(wx,[1,size(wx,2)*10]) ; 
    reswy = imresize(wy,[1,size(wy,2)*10]) ; 
    resdists = mat2gray(sqrt((reswx-screenRect(3)./2).^2 + (reswy - screenRect(4)./2).^2)) ;

    icount = 1 ; clear newdists ; 
    for i=1:length(resdists)
        if 1-resdists(i) > rand
            newdists(icount) = resdists(i) ; 
            newreswx(icount) = reswx(i) ; 
            newreswy(icount) = reswy(i) ; 
            icount = icount + 1 ; 
        end
    end
    wx = imresize(newreswx,[1,length(wx)]) ; 
    wy = imresize(newreswy,[1,length(wy)]) ; 
    dists = imresize(newdists,[1,length(dists)])*300 ; 
    finaldists(st,:) = dists ; 
    finalwx(st,:) = wx ; finalwy(st,:) = wy ; 
end
    ...
    while ~DONE             
        ...
        if currentstate == 2 % present the stimulus               
            ...
            if stimarr(stimcount) == 0
                Screen('DrawTexture',w,blacktex,srcRect,rr) ;                
            else
                Screen('DrawTexture',w,whitetex,srcRect,rr) ;
            end         
            Screen('DrawTexture',w,xhairtex) ;  
            Screen('DrawTexture',w,xhairsmalltex) ;  
           % Screen('DrawTexture',w,circmasktex,[],rr) ;            
