...
%{
randarrs = zeros(400,400,100); 
[xg,yg] = meshgrid(-100:.5:99.5,-100:.5:99.5); sinx = sin(xg); 
icount = 1 ; inds = randperm(numel(sinx));
prevnpix = 1; prevsinx = sinx; 
for i=0.001:.01:1
   npix = i*numel(sinx); 
   npixincr = npix-prevnpix; 
   newsinx = prevsinx; 
   newsinx(inds(prevnpix:prevnpix+800)) = sinx(inds(prevnpix+npixincr/2:prevnpix+npixincr/2+800)); 
   newsinx(inds(prevnpix+npixincr/2:prevnpix+npixincr/2+800)) = sinx(inds(prevnpix:prevnpix+800)); 
   randarrs(:,:,icount) = newsinx; icount = icount+1; 
   prevsinx = newsinx; 
   prevnpix = npix; 
end
doublerandarrs = zeros(size(randarrs,1),size(randarrs,2),size(randarrs,3)*2); 
doublerandarrs(:,:,1:end/2) = randarrs; 
for i=1:size(randarrs,3) ; doublerandarrs(:,:,end/2+i) = randarrs(:,:,end-i+1) ; end

randmask = zeros(size(xg,1),size(xg,2),2) ;
for i=1:size(doublerandarrs,3); 
randmask(:,:,1) = uint8(mat2gray(doublerandarrs(:,:,i))*255) ; 
randmask(:,:,2) = white ; 
randmasktex(i) = Screen('MakeTexture',w,randmask) ;
end
%}
%{
contarrs = zeros(400,400,100); 
[xg,yg] = meshgrid(-100:.5:99.5,-100:.5:99.5); sinx = sin(xg); icount =1 ; 
% 0% contrast = gray = 127.5 + sinx*127.5*i; 
for i=0.001:0.01:1
    contarrs(:,:,icount) = 127.5+sinx*127.5*i; 
    icount = icount + 1; 
end
doublecontarrs = zeros(size(contarrs,1),size(contarrs,2),size(contarrs,3)*2); 
doublecontarrs(:,:,1:end/2) = contarrs; 
for i=1:size(contarrs,3) ; doublecontarrs(:,:,end/2+i) = contarrs(:,:,end-i+1) ; end
contmask = zeros(size(xg,1),size(xg,2),2) ;
for i=1:size(doublecontarrs,3); 
contmask(:,:,1) = uint8((doublecontarrs(:,:,i))) ; 
contmask(:,:,2) = white ; 
