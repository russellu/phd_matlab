...
%{
resbasechans = reshape(basechans,[64,numel(basechans(1,:,:))]) ; 
[weights,sphere] = runica(resbasechans,'maxsteps',128) ; 
acts = weights*sphere*resbasechans ; resacts = reshape(acts,size(allchans)) ; 
winv = pinv(weights*sphere) ; 
mresacts = squeeze(mean(resacts,3)) ; 
skews = skewness(mresacts,[],2) ; 
clear corrs
for i=1:size(resacts,1) ; corrs(i) = corr2(squeeze(mean(resacts(i,:,:),3)),squeeze(mean(basechans(64,:,:),3))) ; end

bads = find(abs(corrs)>.3) ; 
revacts = acts ; revacts(bads) = 0 ; 
invrevacts = winv*revacts ; resinv = reshape(invrevacts,size(allchans)) ; 
figure,for i=1:64  ; subplot(5,13,i) ; imagesc(log(abs(squeeze(resinv(i,:,:))'))) ; axis xy ; end
%}
...
%for i=1:64 ; rt = eegfiltfft(squeeze(restracts(i,:,:))',1/0.693,0.02,1.5) ; restracts(i,:,:) = rt' ; end
%{
clear xcorrs 
for i=1:64 ; 
    for j=1:125
        xcorrs(i,j,:) = xcorr(mts(35:end-35),squeeze(restracts(i,35:end-35,j)),25,'coeff') ; 
    end
end
figure,for i=1:64 ; subplot(5,13,i) ; imagesc(squeeze(xcorrs(i,:,:)),[-.5,.5]) ; axis xy ; title(i) ; end
%}
...
for i=1:64 ; subplot(5,13,i) ; imagesc(squeeze(mean(xcorrs(:,i,:,:),1))) ; axis xy ; end
